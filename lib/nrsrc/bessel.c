...
double dbessk0(double x)
{
  ...
  if (x <= 2.0) {    // Polynomial fit
    y = x*x/4.0;
    ans = (-log(x/2.0)*dbessi0(x)) + (-0.57721566 + y*(0.42278420 + y*(0.23069756 + y*(0.3488590e-1 + y*(0.262698e-2 + y*(0.10750e-3 + y*0.74e-5))))));
  }
  else {
    y = 2.0/x;
...
double dbessk1(double x)
{
  ...
  if (x <= 2.0) {    // Polynomial fit
    y = x*x/4.0;
    ans = (log(x/2.0)*dbessi1(x)) + (1.0/x)*(1.0 + y*(0.15443144 + y*(-0.67278579 + y*(-0.18156897 + y*(-0.1919402e-1 + y*(-0.110404e-2 + y*(-0.4686e-4)))))));
  }
  else {
    y = 2.0/x;
